...
- (void)viewDidAppear:(BOOL)animated{
    ...
    [date loadPublicDeviceWithBuildingId:self.buildingId Token:self.token];
    [date loadPrivateDeviceWithMobile:self.mobile BuildingId:self.buildingId Token:self.token];
    [self loadDate];
}

- (void)loadDate{
    CacheDate *date = [CacheDate new];
    //self.macStr = date.mac;
    self.deviceKey = date.deviceKey;
    self.manufacturerId = date.manufacturerId;
